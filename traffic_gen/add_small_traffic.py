"""
Add fixed-size background flows to an existing traffic file.

Reads a traffic file (generated by traffic_gen.py) and interleaves
Poisson-distributed background flows with random src/dst. The merge
is streaming — only one existing flow and nhost heap entries are in
memory at any time.

Output format is the same as traffic_gen.py:
    <total_flows>
    <src> <dst> <priority> <port> <size_bytes> <start_time_seconds>

Usage:
    uv run traffic_gen/add_small_traffic.py \\
        -i traffic.txt -n 10 -l 0.1 -b 10G -o traffic_with_bg.txt

Options:
    -i  Input traffic file (required)
    -n  Number of hosts (required)
